...
public class GlowingTige : MonoBehaviour
{
    // Start is called before the first frame update
    private Animator animator;

    public GameObject BacLimules;
    ...
    {
        if (other.CompareTag("TrashBin") & CheckSpeed() > 0)
        {
            if (!active)
            {
    ...
    float CheckSpeed()
    {
        ...
        if (BacLimules != null)
        {
            MoveTrashBox moveTrashBox = BacLimules.GetComponent<MoveTrashBox>();
            if (moveTrashBox != null)
            {

                // Return the absolute value of the speed
                return Mathf.Abs(moveTrashBox.Speed);
            }
            else
            {
